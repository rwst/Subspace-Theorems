...
# 1. Import boundary. The library may not depend on the roadmap: `Roadmap/Suggested.lean` is
...
# after the fact. The roadmap states the targets; the library proves them.
...
# the build unrepresentative of what the file needs, and hides the dependency the module doc should
# be stating. Fine-grained imports only. (`Roadmap/Suggested.lean` does use it, and is not scanned.)
if grep -nE '^[[:space:]]*(public[[:space:]]+)?import[[:space:]]+Mathlib[[:space:]]*$' "${files[@]}"; then
  echo "guards: $LIB/ may not \`import Mathlib\`; import the modules the file actually uses." >&2
  status=1
fi

# 4. No wrapper namespace. Declarations go into Mathlib's root namespaces — `Polynomial.mulHeight`,
# `Submodule.mulHeight`, `NumberField.absMulHeight`, as the roadmap pins them — never inside a
# `namespace ArithmeticHeights`. This is also the local form of Tau Ceti's `lint-dot-notation`,
# which is NOT ported: that lint finds a Mathlib type's namespace nested inside the *project*
# namespace, where dot notation silently stops elaborating. Tau Ceti wraps its library in
# `namespace TauCeti` and therefore needs the full search; here the wrapper itself is the thing
# forbidden, and one `grep` settles it.
if grep -nE '^namespace ArithmeticHeights\b' "${files[@]}"; then
