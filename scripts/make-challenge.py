...
DEV_ROOTS = ("ArithmeticHeights", "DiophantineApproximation")
DATA = os.path.join(ROOT, ".lake", "challenge-data")
THEOREMS = os.path.join(ROOT, "comparator", "theorems.txt")
...
def challengeOwner (env : Environment) (n : Name) : Option Name := do
  ...
  if [`ArithmeticHeights, `DiophantineApproximation].any (·.isPrefixOf m) then some m else none
...
def main():
    ...
    stale = []
    for d, _, fs in os.walk(os.path.join(ROOT, "Challenge")):
        for f in fs:
    ...
    roots = [mm.group(1) for mm in (re.match(r"^import (\S+)", l)
             for l in open(os.path.join(ROOT, "Solution.lean"))) if mm]
    ...
    with open(os.path.join(ROOT, "ChallengeFlat.lean"), "w") as f:
        f.write(HEADER + "module\n\n" + "".join(
            f"{'public ' if p else ''}import {i}\n" for i, p in flat_list)
            + "\n" + FLAT_DOC + "\n".join(parts) + "\n")

    for chal, name in [("Challenge", "std3.json"), ("ChallengeFlat", "std3-flat.json")]:
        cfg = {"challenge_module": chal, "solution_module": "Solution",
    ...
    flat = open(os.path.join(ROOT, "ChallengeFlat.lean"), encoding="utf-8").read()
    print(f"make-challenge: {len(theorems)} theorems, {len(order)} challenge modules; "
          f"ChallengeFlat.lean is {flat.count(chr(10))} lines, {len(flat.encode())} bytes")
    for s in stale:
